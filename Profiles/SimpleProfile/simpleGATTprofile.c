...
static uint8 simpleProfileChar3[SIMPLEPROFILE_CHAR3_LEN] = { 0, 1, 2, 3, 4, 5 };
...
static uint8 simpleProfileChar6[SIMPLEPROFILE_CHAR6_LEN] = { 0, 0, 0, 0, 0, 0, 0, 0};
  ...
  switch ( param )
  {
    ...
    case SIMPLEPROFILE_CHAR3:
      if ( len == sizeof ( uint8 ) ) 
      {
        ...
        VOID osal_memcpy( simpleProfileChar3, value, SIMPLEPROFILE_CHAR5_LEN );
      }
      else
      {
        ret = bleInvalidRange;
                            ...
                            uint8 *pValue, uint8 *pLen, uint16 offset, uint8 maxLen )
{
  ...
  HalLcdWriteString( "read attr!", HAL_LCD_LINE_3 );

  sprintf(str, "read attr!%4d %4d %4d \r\n", 
		    pAttr->permissions,*pValue,maxLen,connHandle);
  NPI_WriteString(str);
#endif
                                 ...
                                 uint8 *pValue, uint8 len, uint16 offset )
{
  ...
  sprintf(str, "22test:%4d %4d %4d %4d %4d\r\n", 
           pAttr->type.len, pAttr->type.uuid[0], pAttr->type.uuid[1], len, offset);
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    case SIMPLEPROFILE_CHAR3_UUID:
        ...
        {
          uint8 *pCurValue = (uint8 *)pAttr->pValue;        
          osal_memcpy( pCurValue, pValue, SIMPLEPROFILE_CHAR3_LEN );
      ...
      case SIMPLEPROFILE_CHAR6_UUID:
        ...
        if ( status == SUCCESS )
        {
          ...
          //simpleBLE_GetLicenseID(pValue, licenseID);
          
          osal_memcpy( pCurValue, licenseID, 8 );
