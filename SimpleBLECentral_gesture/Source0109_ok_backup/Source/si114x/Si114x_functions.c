...
s16 _sendCmd(HANDLE si114x_handle, u8 command)
{
    ...
    while(1)
    {
        ...
        {
			retval= Si114xReadFromRegister(si114x_handle, REG_COMMAND);	
			NPI_WriteStrValue("no response! command:%d\r\n", retval);
        }
    }
    return 0;
}
...
s16 _PsAlsPause (HANDLE si114x_handle) 
{
