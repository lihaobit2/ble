...
uint16 SimpleBLECentral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & START_DEVICE_EVT )
  {
    ...
    osal_start_timerEx( simpleBLETaskId, OTC_SENSOR_UPDATE_EVT, OTC_UPDATE_PERIOD );
  ...
  if ( events & OTC_SENSOR_UPDATE_EVT )
  {
    static uint16 updateCnt;
    // Restart timer
    if ((OTC_UPDATE_PERIOD))
    {
      osal_start_timerEx( simpleBLETaskId, OTC_SENSOR_UPDATE_EVT, OTC_UPDATE_PERIOD );
    }
	...
	{   
		#if 1
	    sprintf(astr, "cnt:%d,ps:%d,%u,%d,vis:%d,ir:%d,\entry_time:%d,%d,%d,exit_time:%d,%d,%d\r\n",\
	    updateCnt,Si114xsamples.ps1,Si114xsamples.ps2,Si114xsamples.ps3,\
	    Si114xsamples.vis, Si114xsamples.ir,\
		Si114xsamples.ps_entry_time[0],Si114xsamples.ps_entry_time[1],Si114xsamples.ps_entry_time[2],\
	    Si114xsamples.ps_exit_time[0],Si114xsamples.ps_exit_time[1],Si114xsamples.ps_exit_time[2]);
		NPI_WriteString(astr);		 
		#endif
		#if 0
	    sprintf(astr, "stable flag:%d,state:%d,%d,%d,ps_time:%d,%d,%d\r\n",\
		Si114xsamples.isStableCnt,\
	    Si114xsamples.ps_state[0],Si114xsamples.ps_state[1],Si114xsamples.ps_state[2],\
		Si114xsamples.ps_time[0],Si114xsamples.ps_time[1],Si114xsamples.ps_time[2]);

		NPI_WriteString(astr);
		#endif
		#if 0
	    sprintf(astr, "Cnt:%d,data:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\r\n",\
				updateCnt, g_Data_Buffer[0],g_Data_Buffer[1],\
			    g_Data_Buffer[2],g_Data_Buffer[3],g_Data_Buffer[4],\
	    		g_Data_Buffer[5],g_Data_Buffer[7],g_Data_Buffer[7],\
	    		g_Data_Buffer[8],g_Data_Buffer[9],g_Data_Buffer[10],\
	    		g_Data_Buffer[11],g_Data_Buffer[12]);
		NPI_WriteString(astr);		
		#endif
    ...
    return (events ^ OTC_UPDATE_PERIOD);
  }  
